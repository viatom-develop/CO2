...
public class CRCUitl {
    ...
    public static boolean CRC8(byte[] pData) {
        try {
            byte index = 0, nCrc = 0, nTemp = 0;
            //协议里 第三位是长度
            int Len = 0x00ff & (pData[2] - 1);

            for (index = 0; index < Len; index++) {
                nTemp = pData[index];
                nCrc = CRCTable[0x00ff & (nCrc ^ nTemp)];
            }
            if (nCrc == pData[pData.length - 1] && (Len + 1) == pData.length) {
                return true;
            }
            return false;
        } catch (Exception e) {
    ...
    public static void main(String[] args) {
        byte[] bytes = new byte[]{(byte) 0xAA, (byte) 0x55, (byte) 0x19, (byte) 0x23, (byte) 0xF3, (byte) 0x04, (byte) 0x01, (byte) 0x02
                ...
                , (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x8F};
        System.out.println(CRCUitl.CRC8(bytes));
    }
